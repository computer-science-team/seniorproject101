  ...
  <head>
	...
	<div class="container">
		...
		if($foundRows > 0)
		{
		    ...
		    echo "<table border='1'>";
		    echo "<tr><td>Name</td><td>Category</td><td>Website</td><td>Add to Toolkit</td></tr>";
		    while($row=mysqli_fetch_assoc($queryResult)){
		        
		           
		        //echo "<tr><td>{$row['toolname']}</td><td>{$row['category']}</td><td>{$row['url']}</td><td>{$row['url']}</td>";
		        echo "<tr><th>{$row['toolname']}</th><th>{$row['category']}</th><th>{$row['url']}</th><th><input style = 'margin-left: 30%;'type='submit' name='Add' value = 'Add' href = {$row['url']}></th></tr>";
